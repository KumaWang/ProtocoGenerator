...
namespace fastJSON
{
    ...
    public static class JSON
    {
        ...
        public static string ToJSON(RuntimeObject obj, bool quoteName = true, int indentation = 4)
        {
            ...
            WriteObjectString(jsonWriter, obj);

            return textWriter.ToString();
        }

        private static void WriteObjectString(JsonTextWriter jw, RuntimeObject obj)
        {
            switch (obj.Type)
            {
                case RuntimeObjectType.ClrObject:
                    jw.WriteValue("{ClrObject}");
                    break;
                case RuntimeObjectType.ClrDelegate:
                    jw.WriteValue("{ClrDelegate}");
                    break;
                case RuntimeObjectType.Boolean:
                    jw.WriteValue(obj.Boolean);
                    break;
                case RuntimeObjectType.String:
                    jw.WriteValue(obj.String);
                    break;
                case RuntimeObjectType.Null:
                    jw.WriteNull();
                    break;
                case RuntimeObjectType.Function:
                    jw.WriteValue("{Function}");
                    break;
                case RuntimeObjectType.Number:
                    jw.WriteValue(obj.Number);
                    break;
                case RuntimeObjectType.Object:
                    ...
                    {
                        float v = 0;
                        if (float.TryParse(obj.Object.Members.First().Key, out v))
                        {
                            isArray = true;
                        }
                    }
                    ...
                    foreach (var member in obj.Object.Members)
                    {
                        ...
                        WriteObjectString(jw, member.Value);
                    }
        ...
        /// <returns></returns>
        public static RuntimeObject Parse(string json)
        {
